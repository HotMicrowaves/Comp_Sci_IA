	...
	if pressed == false:
		...
		if(cards != null && ((cards.size() - 1 == operations.size() and Global.enemy_target != null) || operations.size() == 0 and cards.size() != 0) and Global.energy != 0):
			...
			if(operations.size() != 0):
				print("Slot 1: " + str(get_tree().get_nodes_in_group("Slot1")))
				print("Slot 2: " + str(get_tree().get_nodes_in_group("Slot2")))
				print("Slot 3: " + str(get_tree().get_nodes_in_group("Slot3")))
				print("Slot 4: " + str(get_tree().get_nodes_in_group("Slot4")))
				print("Slot 5: " + str(get_tree().get_nodes_in_group("Slot5")))
